...
def saveSide(df_api):
    ...
    # side_raw 테이블의 가장 마지막 timestamp와 다르면
    if timestamp_now!=timestamp_sideRaw_last:
        ...
        if len(df_changedData)>0:
            ...
            # side_raw 테이블에 저장
            df_changedData.index += len(df_side) # primary key 인 index 중복 방지
...
def compareRawData(df_raw, df_api):
    ...
    df_recent=df_raw.drop_duplicates(['name'],keep='last').drop(['index'],axis=1).reset_index(drop=True)
    ...
    for i in range(len(df_api)):
        ...
        df_api_tmp = df_api_withoutTimestamp[df_api_withoutTimestamp['name']==df_recent_withoutTimestamp.loc[i,'name']]
        isSameData = df_recent_withoutTimestamp.iloc[i]==df_api_tmp # series bool
